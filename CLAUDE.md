...
- **Static menu** — three items: "Show Radar" (focuses the window), "Run briefing now" (emits `tray://run-briefing`; `App.jsx` listens and fires `onBriefing`), "Quit Radar". The menu is built once in `tray::init`; nothing rebuilds it because nothing about it is state-dependent. Left-click on the tray icon toggles window visibility (`toggle_main_window` — hide if visible+focused, otherwise show+focus); right-click opens the menu.
- **Tooltip-as-dashboard** — all state-driven info lives in the hover tooltip. The frontend composes it in `src/tray.js::buildTooltip`: header line `"Radar — {N} unread · last run {rel}"` (or `"all caught up"`), blank line, then the top 3 article headlines as `"<Source> — <title>"`, each truncated to 72 chars. `pushTrayStatus({unread, lastRunAt, articles})` pushes `{title, tooltip}` to the `set_tray_status` command; the macOS title slot gets just the unread count (or null) so the menu bar stays visually quiet.
- **Close-to-tray** — `lib.rs` intercepts `WindowEvent::CloseRequested`, calls `prevent_close()`, hides the window. Cmd+Q and tray → Quit still `app.exit(0)` normally.
- **Reopen handling** — `RunEvent::Reopen` (macOS-only; fired on notification-click / dock-click / activation) surfaces the hidden window. The match arm is `#[cfg]`-gated because the variant doesn't exist on Windows/Linux.
