...
- **Daily schedule.** `scheduleEnabled` + `scheduleTime` (`HH:MM` local) in prefs. A `useEffect` in `App.jsx` schedules a `setTimeout` at the next occurrence; on fire, it calls `onBriefing` via `onBriefingRef.current` (so the timer doesn't capture stale state). If the app launches *after* today's slot and `archives[0].runAt` is older than that slot, it fires immediately to cover the missed run. The effect re-runs when `archives` changes, which handles rescheduling tomorrow. This is **frontend-only** — it only fires while Radar is open. An OS-level background scheduler (launchd / Task Scheduler / tray resident) is not implemented.
